...
def plot_price_and_rate(series, price_col, price_label, title):
    """
    Цена и ставки на одном графике с двумя осями
    Слева цена за м², справа проценты годовых
    """
...
def plot_lagged_correlation(table, title):
    ...
    colors = ['crimson' if v < 0 else 'steelblue' for v in table['Корреляция']]
...
def plot_forecast(series, col, forecast, title, history_months=24):
    ...
    plt.figure(figsize=(12, 5))
    plt.plot(series.index, series[col], color='steelblue', linewidth=1.8, label='Факт')

    trend_start = series.index[-history_months]
    plt.axvspan(trend_start, series.index[-1], color='steelblue', alpha=0.08,
                label=f'Участок построения тренда ({history_months} мес)')

    bridge = pd.concat([series[col].iloc[[-1]], forecast])
    plt.plot(bridge.index, bridge.values, color='crimson', linewidth=2,
